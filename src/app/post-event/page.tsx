  ...
  const postAuthData = async () => {
    try {
      const response = await axios.post(
        process.env.NEXT_PUBLIC_API_URL + "checkAuth",
        authObject
      );
    } catch {
      if (isClient) {
    ...
    switch (type) {
      case "name":
        if (eventObject[type].length < 3) {
          ...
          setInvalidFields(() => {
            return { ...invalidFields, name: true };
          });
          setShowErrorDialogue(true);
          return false;
        } else
          setInvalidFields(() => {
            return { ...invalidFields, name: false };
          });
        break;
      case "description":
        if (eventObject[type].length < 10) {
          setInvalidFields(() => {
            return { ...invalidFields, description: true };
          });
          setShowErrorDialogue(true);
          return false;
        } else
          setInvalidFields(() => {
            return { ...invalidFields, description: false };
          });
        break;
      case "town":
        if (eventObject[type].length < 3) {
          setInvalidFields(() => {
            return { ...invalidFields, town: true };
          });
          setShowErrorDialogue(true);
          return false;
        } else
          setInvalidFields(() => {
            return { ...invalidFields, town: false };
          });
        break;
      case "contact":
        if (!eventObject[type].includes("@")) {
          setInvalidFields(() => {
            return { ...invalidFields, contact: true };
          });
          setShowErrorDialogue(true);
          return false;
        } else
          setInvalidFields(() => {
            return { ...invalidFields, contact: false };
          });
        break;
      default:
        return true;
      ...
      <form onSubmit={handleSubmit} className={classes.formWrapper}>
        {showErrorDialogue && (
          <div className={classes.errorBox}>
            There was an error in the details you submitted
          </div>
        )}
        ...
        <input
          ...
          className={`${classes.formField} ${
            invalidFields.name ? classes.inputInvalid : ""
          }`}
        ></input>
        ...
        <input
          ...
          onBlur={handleValidate}
          className={`${classes.formField} ${
            invalidFields.description ? classes.inputInvalid : ""
          }`}
        ></input>
        <label htmlFor="town" className={classes.inputLabel}>
          Town
        </label>
        <input
          type="text"
          ...
          onBlur={handleValidate}
          className={`${classes.formField} ${
            invalidFields.town ? classes.inputInvalid : ""
          }`}
        ></input>
        <label htmlFor="contact" className={classes.inputLabel}>
          Contact Email
        </label>
        <input
          type="text"
          ...
          onBlur={handleValidate}
          className={`${classes.formField} ${
            invalidFields.contact ? classes.inputInvalid : ""
          }`}
        ></input>
